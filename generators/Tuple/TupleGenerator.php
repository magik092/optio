...
class TupleGenerator
{
    ...
    public const TUPLE_NAMESPACE = 'Optio\Tuple';
    ...
    public function prepareTuples(int $maxTupleSize = 8): void
    {
        ...
        $this->classPersister->ensureDirectoryExists(self::TEMP_DIRECTORY_NAME);
        ...
        for ($size = 0; $size <= $maxTupleSize; ++$size) {
            ...
            $preparedClassName = $class->getName();
            if (null === $preparedClassName) {
                throw new \RuntimeException(sprintf('Generated class for tuple size %d has no name.', $size));
            }
            ...
            $this->preparedClasses[] = $preparedClassName;
        }
    }
